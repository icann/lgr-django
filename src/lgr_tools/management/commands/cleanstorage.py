...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        if 2 * STORAGE_DURATION > SESSION_COOKIE_AGE:
        ...
        for session in sessions:
            ...
            if 'storage' not in session_data:
                continue
            storage_path = os.path.join(TOOLS_OUTPUT_STORAGE_LOCATION,
            ...
            if os.path.getmtime(storage_path) + STORAGE_DURATION < time.time():
                try:
            ...
            for out_fname in os.listdir(storage_path):
                ...
                if os.path.getctime(out_fpath) + STORAGE_DURATION < time.time():
